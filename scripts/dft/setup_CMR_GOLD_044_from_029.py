...
SMILES_044 = "O=C1OC2=CC=CC=C2C(C3=NOC(C)=N3)=C1"

EXPECTED_FORMULA = "C12H8N2O3"
EXPECTED_N = 2
EXPECTED_TOTAL_ATOMS_WITH_H = 25

OLD_WORKFLOW = PROJECT / "cmr029_workflow.py"
NEW_WORKFLOW = PROJECT / "cmr044_workflow.py"
...
def create_workflow():
    ...
    s = s.replace("CMR_GOLD_029", "CMR_GOLD_044")

    # 029 SMILES yerine 044 SMILES koy
    old_smiles = "O=C1OC2=C(C=CC(OCC(O)CN3C=NC4=C3C=CC=C4)=C2)C(C)=C1"
    s = s.replace(old_smiles, SMILES_044)

    # 32 GB RAM için güvenli ayarı garanti et
...
def create_start_geometry():
    mol0 = Chem.MolFromSmiles(SMILES_044)
    ...
    print("\nCMR_GOLD_044 başlangıç geometrisi oluşturuldu ve doğrulandı.")
...
def main():
    ...
    print("\nSonraki komutlar:")
    print("python cmr044_workflow.py prepare")
    print("python cmr044_workflow.py status")
    print("python cmr044_workflow.py run-opt")
