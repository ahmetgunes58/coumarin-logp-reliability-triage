...
REPORT_DIR = MOL_DIR / "extracted_data"

XYZ_RDKIT = INPUT_DIR / f"{MOL_ID}_start_rdkit_mmff.xyz"
XYZ_ORCA = INPUT_DIR / f"{MOL_ID}_start.xyz"
SDF_OUT = INPUT_DIR / f"{MOL_ID}_start_rdkit_mmff.sdf"
REPORT_TXT = REPORT_DIR / f"{MOL_ID}_start_xyz_validation_report.txt"
REPORT_CSV = REPORT_DIR / f"{MOL_ID}_start_xyz_validation_report.csv"


def ensure_dirs():
    for d in [INPUT_DIR, GEOM_DIR, REPORT_DIR]:
...
def write_xyz(mol, path):
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"{MOL_ID} RDKit ETKDGv3 + MMFF94 starting geometry; neutral singlet\n")
...
def main():
    ...
    if mol0 is None:
        raise RuntimeError("SMILES okunamadı. SMILES stringini kontrol et.")
    ...
    params.randomSeed = 29029
    params.pruneRmsThresh = 0.25
    params.numThreads = 0
    params.useRandomCoords = False

    n_confs_requested = 100
    ...
    if not AllChem.MMFFHasAllMoleculeParams(mol):
        raise RuntimeError("MMFF parametreleri bu molekül için eksik. UFF fallback gerekebilir.")
    ...
    # En düşük enerjili konformeri tek konformer olarak al
    ...
    with REPORT_TXT.open("w", encoding="utf-8") as f:
        f.write(f"{MOL_ID} RDKit-MMFF starting geometry validation report\n")
        f.write("=" * 72 + "\n\n")
        for k, v in checks.items():
            f.write(f"{k}: {v}\n")

        f.write("\nExpected key checks:\n")
        f.write("- Formula should be C20H18N2O4\n")
        f.write("- Formal charge should be 0\n")
        f.write("- N_count should be 2\n")
        f.write("- Total atoms with explicit H should be 44\n")
    ...
    print("\nCMR_GOLD_029 başlangıç geometrisi oluşturuldu ve doğrulandı.")
    ...
    if formula != "C20H18N2O4":
        print("\nUYARI: Formula beklenen C20H18N2O4 ile uyuşmuyor.")
    if formal_charge != 0:
        print("\nUYARI: Molekül nötral değil.")
    if n_count != 2:
        print("\nUYARI: N count 2 değil.")
    if total_atoms != 44:
        print("\nUYARI: Explicit H ile toplam atom sayısı 44 değil.")
    if min_nb_dist < 0.75:
    ...
    print("\nSonraki adım:")
    print("python cmr029_workflow.py prepare")
    print("python cmr029_workflow.py status")
    print("python cmr029_workflow.py run-opt")
