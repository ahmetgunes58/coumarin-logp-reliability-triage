# -*- coding: utf-8 -*-
"""
Generate Figure S5 trial version: Auxiliary molecular electrostatic potential maps
for CMR_GOLD_055 and CMR_GOLD_079 with explicit 3D-oriented molecular poses.

This is a non-destructive trial script.
It does NOT overwrite the current final Figure S5 files.

Scientific rendering protocol:
    - MEP mapped onto electron-density isosurface
    - electron-density isovalue = 0.004 a.u.
    - common electrostatic-potential scale = -0.08 to +0.08 a.u.
...
Trial outputs:
    figures/supporting/Figure_S5_auxiliary_MEP_maps_3Dpose_trial.png
    figures/supporting/Figure_S5_auxiliary_MEP_maps_3Dpose_trial.tiff
    figures/supporting/Figure_S5_auxiliary_MEP_maps_3Dpose_trial.pdf
    data/processed/Figure_S5_auxiliary_MEP_maps_3Dpose_trial_summary.txt
...
# These rotations are applied directly to both:
#   1. molecular coordinates
#   2. electron-density/MEP surface mesh
#
# Purpose:
#   To avoid a flat, nearly 2D front-view appearance and obtain a clearer
#   3-axis molecular pose.
#
# Adjust only these values if the molecule still looks too flat.
ORIENTATION = {
    "CMR_GOLD_055": {"rot_x": 0.0, "rot_y": 0.0, "rot_z": 0.0},
    "CMR_GOLD_079": {"rot_x": 0.0, "rot_y": 0.0, "rot_z": -8.0},
}
...
OUTPUT_PNG = FIG_DIR / "Figure_S5_auxiliary_MEP_maps_3Dpose_trial.png"
OUTPUT_TIFF = FIG_DIR / "Figure_S5_auxiliary_MEP_maps_3Dpose_trial.tiff"
OUTPUT_PDF = FIG_DIR / "Figure_S5_auxiliary_MEP_maps_3Dpose_trial.pdf"
OUTPUT_SUMMARY = DATA_DIR / "Figure_S5_auxiliary_MEP_maps_3Dpose_trial_summary.txt"
...
# Slightly larger margin than the original version to avoid edge clipping.
CAMERA_MARGIN = 1.12

# Oblique camera position. This helps the viewer perceive x/y/z directions.
CAMERA_Z_FACTOR = 2.75
CAMERA_X_TILT = 0.00
CAMERA_Y_TILT = 0.00

# Keep False first for publication-style controlled projection.
# Set True only if you still need stronger visual 3D depth.
...
def rotate_points(
    ...
    rotation = rotation_matrix_xyz(rot_x, rot_y, rot_z)
    shifted = points - center
    rotated = shifted @ rotation.T
    return rotated + center


def apply_orientation_to_surface_and_coords(
    surface: pv.PolyData,
    coords: np.ndarray,
    ...
    rot_x = float(orientation.get("rot_x", 0.0))
    rot_y = float(orientation.get("rot_y", 0.0))
    rot_z = float(orientation.get("rot_z", 0.0))

    rotated_coords = rotate_points(coords, center, rot_x, rot_y, rot_z)

    rotated_surface = surface.copy(deep=True)
    rotated_surface.points = rotate_points(
        rotated_surface.points,
        center,
        rot_x,
        rot_y,
        rot_z,
    ...
    # Recompute normals after rotation so lighting remains physically coherent.
    rotated_surface = safe_compute_normals(rotated_surface)

    return rotated_surface, rotated_coords
...
def prepare_molecule(cfg: Dict) -> Dict:
    ...
    surface, coords = apply_orientation_to_surface_and_coords(
        surface,
        coords,
        orientation,
    )
...
def write_summary(molecules: List[Dict], common_parallel_scale: float) -> None:
    with OUTPUT_SUMMARY.open("w", encoding="utf-8") as f:
        f.write("Figure S5 auxiliary MEP rendering summary - 3D pose trial\n")
        f.write("=" * 72 + "\n")
        ...
        f.write(f"Camera margin: {CAMERA_MARGIN:.6f}\n")
        f.write(f"Camera Z factor: {CAMERA_Z_FACTOR:.6f}\n")
        f.write(f"Camera X tilt: {CAMERA_X_TILT:.6f}\n")
        f.write(f"Camera Y tilt: {CAMERA_Y_TILT:.6f}\n")
        f.write(f"Perspective projection: {USE_PERSPECTIVE_PROJECTION}\n")
        ...
        for mol in molecules:
            ...
            f.write(
                "  orientation  : "
                f"rot_x={orientation.get('rot_x', 0.0):.3f}, "
                f"rot_y={orientation.get('rot_y', 0.0):.3f}, "
                f"rot_z={orientation.get('rot_z', 0.0):.3f}\n"
            )
...
def main() -> None:
    print("=" * 72)
    print("Preparing Figure S5 auxiliary MEP maps - 3D pose trial")
    print("Molecules: CMR-GOLD-055 and CMR-GOLD-079")
    print("=" * 72)
    ...
    for cfg in MOLECULES:
        ...
        print(f"  pose     : {ORIENTATION.get(cfg['key'])}")
    ...
    print(f"Perspective projection: {USE_PERSPECTIVE_PROJECTION}")
    ...
    print("Assembling trial PNG, TIFF, and PDF...")
